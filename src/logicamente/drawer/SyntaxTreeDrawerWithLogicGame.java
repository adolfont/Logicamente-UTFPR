...
public class SyntaxTreeDrawerWithLogicGame extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		SyntaxTreeDrawerWithLogicGame std = new SyntaxTreeDrawerWithLogicGame();
	...
	private static void initFromCommandLine(String[] args,
			SyntaxTreeDrawerWithLogicGame std) {
		String formula;
		if (args.length == 1) {
			formula = args[0];
			std = new SyntaxTreeDrawerWithLogicGame();
	...
	public SyntaxTreeDrawerWithLogicGame() {
		parser = new Parser();
		this.width = 700;
		this.height = 500;
	...
	private void btnMaxParenthesesClick() {
		if (formula == null) {
			logicGamesResult.setText("Formula was not defined!");
		} else if (logicGamesMaxParentheses.getText()
				.equals(formula.toString())) {
			logicGamesResult.setText("Right answer!");
		} else {
//			logicGamesResult.setText("Wrong! Should be " + formula.toString());
			logicGamesResult.setText("Wrong answer! Try again!");
		}
	}
	...
	private void drawScreen() {
		...
		southPanel.add(logicGamesPanel, BorderLayout.SOUTH);
		getContentPane().add(southPanel, BorderLayout.SOUTH);
	}

	private void addLogicGames() {
		logicGamesPanel = new JPanel();
		logicGamesPanel.setBorder(BorderFactory.createLineBorder(Color.red));
		logicGamesPanel.setLayout(new GridLayout(2,2));
		logicGames = new JLabel("Write the same formula with all parentheses: ", JLabel.CENTER);
		logicGamesResult = new JLabel("", JLabel.CENTER);
		logicGamesMaxParentheses = new JTextField(10);
		btnMaxParentheses = new JButton();
		btnMaxParentheses.setText("Click to verify your answer");
	...
	private void setTopAndDrawingPanelProperties() {
		...
		forbidVerifyFormulaWhileTreeNotGenerated();
		topPanel.setBorder(javax.swing.BorderFactory.createEtchedBorder());
		drawingPanel.setBounds(0, 0, 500, 500);
	}

	private void forbidVerifyFormulaWhileTreeNotGenerated() {
		inputFormulaTextField.getDocument().addDocumentListener(new DocumentListener() {
			@Override
	...
	public void setFormula(String string) {
		...
		if (result.parseCorrect()) {
			...
			logicGamesMaxParentheses.setText("");
			logicGamesResult.setText("");
		} else {
			formula = null;
			if (grid != null)
				grid.clear();
