...
public class SyntaxTreeDrawer extends JFrame {
	...
	private void drawSyntaxTreeV3(int xPrev, int yPrev, int x, int y,
		...
		if (formula instanceof AtomicFormula) {
			...
			if (xPrev != 0) {
				grid.drawLine(xPrev, yPrev, x+ left.getComplexity(), y);
			}
			grid.drawNode(x + left.getComplexity(), y,
					((CompositeFormula) formula).getConnective().toString());
			drawSyntaxTreeV3(x + left.getComplexity(), y, x, y + 1, left);
			if (right != null)
				drawSyntaxTreeV3(x + left.getComplexity(), y, x
						+ left.getComplexity() + 1, y + 1, right);
		}
	...
	private void recursivelyFillScreen(Formula formula) {
		...
		if (formula instanceof AtomicFormula) {
			...
			getGraphics().drawOval(leftBorder + x, topBorder, diameter,
					diameter);
			...
			if (right != null)
				recursivelyFillScreen2(baseX, baseY, right);
		}
	}

	private void recursivelyFillScreen2(int originalBaseX, int originalBaseY,
			Formula formula) {
